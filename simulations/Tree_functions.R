...
sim_study <- function(d = 10, n = 100, p = NULL, 
  ...
  ##        to the simulation of the model
...
mst_HR_parallel <- function (data, p = NULL, cens = FALSE) {
  if (!is.null(p)) {
  ...
  if (cens) {
    ...
    bivLLH <- foreach(i = 1:nrow(res), .combine = cbind, 
                      .packages = "graphicalExtremes") %dopar% {
                        x <- res[i, ]
                        fmpareto_obj <- graphicalExtremes:::fmpareto_HR(data = data.std[, 
                                                                    x], init = G.emp[x[1], x[2]], cens = cens)
  ...
  if (!cens) {
    bivLLH <- foreach(i = 1:nrow(res), .combine = cbind, 
      ...
      {
        x <- res[i, ]
        par.est <- graphicalExtremes:::fmpareto_HR(data = data.std[, x], 
                                                   init = G.emp[x[1], x[2]],
                                                   cens = cens)$par
        
        llh_hr <- logLH_HR(data = data.std[, x], 
                           Gamma = graphicalExtremes:::par2Gamma(par.est)) + 
  ...
  bivLLH.mat <- graphicalExtremes:::par2Gamma(bivLLH["llh_hr", ])
  mst.tree = igraph::mst(graph = graph.full, 
                         weights = -bivLLH.mat[igraph::ends(graph.full,igraph::E(graph.full))],
                         algorithm = "prim")
  mst.tree <- graphicalExtremes:::set_graph_parameters(mst.tree)
  est_Gamma <- graphicalExtremes:::par2Gamma(bivLLH["par", ])
  return(list(tree = mst.tree, 
              Gamma = graphicalExtremes:::complete_Gamma(graph = mst.tree, 
                                                      Gamma = est_Gamma)))
}
